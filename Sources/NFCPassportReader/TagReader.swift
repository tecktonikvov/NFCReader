...
public class TagReader {
    ...
    func readDataGroup( dataGroup: DataGroupId ) async throws -> [UInt8]  {
        ...
        let bench = StepBenchTimer(stepName: " ==== selectFileAndRead \(dataGroup.getName())", isInDebug: true)
        let result = try await selectFileAndRead(tag: tag )
        bench.stop()
        return result
    }
    ...
    func selectFileAndRead( tag: [UInt8]) async throws -> [UInt8] {
        let bench = StepBenchTimer(stepName: " ==== selectFile \(tag)", isInDebug: true)
        ...
        if tag == [0x01, 0x22] {
            length = try! asn1Length([UInt8](resp.data[2..<4]))
        } else {
        ...
        let bench2 = StepBenchTimer(stepName: " ====  while remaining > 0 \(tag)", isInDebug: true)
...
class StepBenchTimer {
    ...
    init(stepName: String, isInDebug: Bool) {
        self.stepName = stepName
        self.isInDebug = isInDebug

        if isInDebug {
            print("Початок \(stepName)")
        }
    ...
    func stop() {
        ...
        if isInDebug {
            print("Кінець", "\(duration)".prefix(4), "\(stepName)")
        }
    }
